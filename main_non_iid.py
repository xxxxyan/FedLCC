import argparse, torch, pickle, os, time, sys, copy,random
...
def fed_train(args, log_txt, model, train_features, dev_features, test_features):  
    ...
    with open(args.dataset_dir +'/%s_non_iid.json'% dataset, "r") as json_file:
        dict_users = json.load(json_file)#non iid
    ...
    net_locals = [copy.deepcopy(net_glob) for i in range(num_users)]
    ...
    for iter in range(int(args.num_train_epochs)):
        ...
        for idxs in user_idxs:
            user_train_features = []
            for idx in dict_users[str(idxs)]:
                user_train_features.append(train_features[idx])
...
def get_args():
    ...
    parser.add_argument("--save_path", default="./save_model/non_iid/", type=str)
    ...
    parser.add_argument("--model_name_or_path", default="/raid/xiaoyan/scibert_scivocab_cased", type=str)
    ...
    parser.add_argument("--fed_algo", type=str, choices=['fed_avg', 'fedlcc', 'fed_attn'], default='fedlcc')
    parser.add_argument('--frac', type=float, default=1.0, help="the fraction of clients: C")
    parser.add_argument("--optimizer", type=str, default="Adam") 
    
    parser.add_argument("--local_epoch", type=int, default=1, help='the number of local epochs: E')#2
...
def main():
    ...
    argument_path = os.path.join(save_path, '%s-frac=%s') %(args.fed_algo, args.frac)
